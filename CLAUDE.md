...
`to-project`, `to-spec`, and `to-tickets` intentionally overlap. When changing any of them, review all three and propagate changes to shared conventions: testing seams, spec sections, single-slice sizing, ticket acceptance criteria, waves and blockers, tracker publication, and design-artifact commits. Preserve the deliberate difference: `to-spec` and `to-tickets` review before publication; `to-project` completes spec and ticket publication before asking for changes. All three remain user-invoked. Keep `to-project` self-contained so it works when installed alone; do not replace its workflow with calls or cross-folder references to the other two skills. Update their README descriptions when behaviour changes. This file is also exposed as `AGENTS.md` for other agents.
