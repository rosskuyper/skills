...
**On GitHub and GitLab**, those templates carry a "PRs as a request surface" flag, defaulted **off** — leave it off and don't raise it; a user who wants external PRs in the triage queue can flip the flag in the file later.
...
**On Linear, the five roles do not all map to labels.** Linear gives every issue one **workflow state** (grouped into the fixed categories `triage`, `backlog`, `unstarted`, `started`, `completed`, `canceled`) *plus* any number of labels, so two of the roles belong to the state dimension:

- `needs-triage` → Linear's native **Triage** inbox where the team has it enabled, otherwise a `backlog`-category state. Not a label.
- `wontfix` → a **`canceled`**-category state (usually `Canceled`). Not a label.
- `needs-info`, `ready-for-agent`, `ready-for-human` → labels, as elsewhere.

Resolve the team's real state names with the Linear MCP server before writing — display names are configurable per team, so don't assume `Canceled` or `Backlog`. Record each role's **mechanism** (state or label) alongside its string in `docs/agents/triage-labels.md`, using the Linear variant of the table in [triage-labels.md](./triage-labels.md). Getting this wrong makes `/triage` try to create a `wontfix` label on a tracker that expresses that as a state.
